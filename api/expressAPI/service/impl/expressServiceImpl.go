...
func (e *ExpressDB) CreateExpress(express *domain.Express) (*domain.Express, error) {
	...
	if err != nil {
		zap.L().Sugar().Errorf("快递新增执行sql错误===%+v", err)
		return nil, err
	}
	id, err := rows.LastInsertId()
	if err != nil {
		zap.L().Sugar().Errorf("快递新增返回新增数据的ID错误===%+v", err)
		return nil, err
	}
	//express.ID = id
	...
	if err != nil {
		zap.L().Sugar().Errorf("获取快递异常===%+v", err)
		return nil, err
	}
	return express, nil
}
...
func (e *ExpressDB) GetExpress(id int64) (*domain.Express, error) {
	...
	if err != nil {
		zap.L().Sugar().Errorf("快递查询构建数据错误===%+v", err)
		return nil, err
	}
...
func (e *ExpressDB) SelectExpressPage(expressName string, pageStr, sizeStr string) ([]*domain.Express, int64, int64, error) {
	...
	zap.L().Sugar().Infof("查询条数sql===%s", countSql)
	err := e.Db.QueryRow(countSql).Scan(&totalRecords)
	if err != nil {
		zap.L().Sugar().Errorf("快递分页sql查询条数错误===%+v", err)
		return nil, 0, 0, err
	}
	...
	zap.L().Sugar().Infof("查询分页sql===%s, offset===%d, size====%d", limitQuerySql, offset, size)
	rows, err := e.Db.Query(limitQuerySql, offset, size)
	if err != nil {
		zap.L().Sugar().Errorf("快递分页sql查询错误===%+v", err)
		return nil, 0, 0, err
	}
	// defer rows.Close()
	...
	if err != nil {
		zap.L().Sugar().Errorf("快递分页构建数据错误===%+v", err)
...
func (e *ExpressDB) SelectExpressPageByParam(param *param.ExpressSearchParam) ([]*domain.Express, int64, int64, error) {
	...
	err := e.Db.QueryRow(countSql).Scan(&totalRecords)
	if err != nil {
		zap.L().Sugar().Errorf("快递分页查询总条数数据转换错误===%+v", err)
		return nil, 0, 0, err
	}
	...
	// 构建 SQL 查询语句
	limitQuerySql := "SELECT id, IFNULL(user_id, ''), IFNULL(express_name, ''), IFNULL(express_number,''), IFNULL(from_name, ''), IFNULL(from_phone, ''), IFNULL(from_address, ''), IFNULL(pickup_code, ''), IFNULL(create_by, ''), IFNULL(DATE_FORMAT(create_time, '%Y-%m-%d %H:%i:%s' ), ''), IFNULL(DATE_FORMAT(update_time, '%Y-%m-%d %H:%i:%s' ), '') FROM tool_express_manage" + buildWhereClauseByParam(param) + buildOrderBy(param.Column, param.Order, "") + " LIMIT ?, ?"
	zap.L().Sugar().Infof("查询分页sql===%s, offset===%d, size====%d", limitQuerySql, offset, size)
	rows, err := e.Db.Query(limitQuerySql, offset, size)
	if err != nil {
		zap.L().Sugar().Errorf("快递分页查询数据列表错误===%+v", err)
		return nil, 0, 0, err
	}
	// todo 这里的rows必须关闭，defer rows.Close()因为用了defer位置不重要，但是使用自定义方法关闭rows，就得注意下关闭调用的顺序
	// defer rows.Close()
	defer RowsClose(rows, "快递分页查询")

	expressesArray, err := e.buildPageData(rows)
	if err != nil {
		zap.L().Sugar().Errorf("快递分页查询数据构建错误===%+v", err)
...
func (e *ExpressDB) buildPageData(rows *sql.Rows) ([]*domain.Express, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			zap.L().Sugar().Errorf("构建快递集合错误===%+v", err)
			return nil, err
		}
	...
	// 检查遍历过程中是否有错误
	if err := rows.Err(); err != nil {
		zap.L().Sugar().Errorf("快递数据遍历错误===%+v", err)
		return nil, err
	}
...
func (e *ExpressDB) DeleteById(id int64) (int64, error) {
	...
	if err != nil {
		zap.L().Sugar().Errorf("快递删除执行错误===%+v", err)
		return 0, nil
	}
	rowsAffected, err := rows.RowsAffected()
	if err != nil {
		zap.L().Sugar().Errorf("快递删除获取执行条数错误===%+v", err)
		return 0, nil
	}
	if rowsAffected == 0 {
...
func (e *ExpressDB) UpdateExpress(express *domain.Express) (int64, error) {
	...
	if err != nil {
		zap.L().Sugar().Errorf("快递更新执行错误===%+v", err)
		return 0, err
	}
	count, err := rows.RowsAffected()
	if err != nil {
		zap.L().Sugar().Errorf("快递更新获取执行条数错误===%+v", err)
		return 0, err
	}
	return count, nil
}
...
func (e *ExpressDB) BatchDeleteByIds(ids []string) (int64, error) {
	...
	zap.L().Sugar().Infof("批量删除sql===%s", deleteSql)
	rows, err := e.Db.Exec(deleteSql)
	if err != nil {
		zap.L().Sugar().Errorf("快递批量删除执行错误===%+v", err)
		return 0, nil
	}
	rowsAffected, err := rows.RowsAffected()
	if err != nil {
		zap.L().Sugar().Errorf("快递批量删除获取执行条数错误===%+v", err)
		return 0, nil
	}
	if rowsAffected == 0 {
		return 0, errors.New("未找到要删除的记录")
...
func (e *ExpressDB) BatchCreateExpress(list []*domain.Express) (int64, error) {
	...
	if err != nil {
		zap.L().Sugar().Errorf("快递批量新增执行错误===%+v", err)
		return 0, err
	}

	count, err := rows.RowsAffected()
	if err != nil {
		zap.L().Sugar().Errorf("快递批量新增获取执行条数错误===%+v", err)
		return 0, err
	}
